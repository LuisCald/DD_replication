...
function compare_to_external_sources(data_dt, ty, func_data, obs_data, user_params, time_params, model_options, type, label, data_bounds=false)
# Comparing our estimates with the observed data, both survey anfunction compare_to_external_sources(data_dt, func_data, user_params, time_params, method_options, type, label, data_bounds=false)
    ...
    compare_to_DFA(deepcopy(data_dt), ty, time_params, type, case, measures, func_dict, gdp_series, tag)
            ...
            for (ext_s, df) in external_sources[obj][meas]
                ...
                for r in all_recons
                    for (k,v) in dist_dict
                        ...
                        v[obj][meas][r]                       = subset_to_cutoff(data_dt[r][ty][meas][obj]["common series"][series[k]], dates_dict, tmin, tmax, obj) 
                        # if ty == "average"
                        ...
                        if length(possible_ext_sources) == 0
                            ...
                            for xx in 1:2
                                ...
                                Plots.plot(xa,
                                    ...
                                    xticks=(xa[1:20:end], [L"%$(date)"[1:5] * "\$" for (_,date) in enumerate(dts[1:20:end])]),
                                    label=L"\textrm{Model}",
                                    lc= :red, 
                                    la = opacities[xx],
                                    xtickfontsize=10,
...
function compare_to_DFA(data_dt, ty, time_params, type, case, measures, func_dict, gdp_series, tag)
    ...
    dfa_dates["tmin"]                = Dict("year" => minimum(wealth_by_income[!, "year"]), "quarter" => minimum(wealth_by_income[wealth_by_income[!, "year"] .== minimum(wealth_by_income[!, "year"]), "quarter"]))
    ...
    dsources        = sort(setdiff(collect(keys(data_dt)), ["consensus"]))
    ...
    for k in collect(dsources)
        ...
        # Subset the DFA data based on the agreeable dates
        wealth_by_incomeₖ[k] = filter(x -> x.dates >= QuarterlyDate(agr_dates[k]["tmin"]["year"], agr_dates[k]["tmin"]["quarter"]), wealth_by_income)
        wealth_by_incomeₖ[k] = filter(x -> x.dates <= QuarterlyDate(agr_dates[k]["tmax"]["year"], agr_dates[k]["tmax"]["quarter"]), wealth_by_incomeₖ[k])
        ...
        if "income" in obs_meas && "wealth" in obs_meas
            ...
            # generate total wealth 
            # transform!(micro_df, [:wealthlevels_bottom40, :wealthlevels_next40, :wealthlevels_top20] => (+) => :tot_wealth)

            # top[k]["shares"]["wealth_by_income"]         = top[k]["levels"]["wealth_by_income"] ./ micro_df[!, "tot_wealth"]
            # bot[k]["shares"]["wealth_by_income"]         = bot[k]["levels"]["wealth_by_income"] ./ micro_df[!, "tot_wealth"]
            # mid[k]["shares"]["wealth_by_income"]         = mid[k]["levels"]["wealth_by_income"] ./ micro_df[!, "tot_wealth"]

            # top[k]["levels"]["wealth_by_income"]         = log_transformation(top[k]["levels"]["wealth_by_income"])
            # bot[k]["levels"]["wealth_by_income"]         = log_transformation(bot[k]["levels"]["wealth_by_income"])
            # mid[k]["levels"]["wealth_by_income"]         = log_transformation(mid[k]["levels"]["wealth_by_income"])
        end
    end
...
function compare_to_data(data_dt, ty, func_data, obs_data, user_params, time_params, model_options, type, label, data_bounds=false)
    ...
    for meas in measures
        ...
        for obj in ["shares", "quantiles"] #TODO: levels?
            ...
            top["consensus"][obj][meas]         = subset_to_cutoff(data_dt["consensus"][ty][meas][obj]["common series"][series["top"]], dates[meas], tmin, tmax, obj, agg_corr)
            bot["consensus"][obj][meas]         = subset_to_cutoff(data_dt["consensus"][ty][meas][obj]["common series"][series["bot"]], dates[meas], tmin, tmax, obj, agg_corr)
            mid["consensus"][obj][meas]         = subset_to_cutoff(data_dt["consensus"][ty][meas][obj]["common series"][series["mid"]], dates[meas], tmin, tmax, obj, agg_corr)
            ...
            for source in data_sources
                ...
                top[source][obj][meas]                   = subset_to_cutoff(data_dt[source][ty][meas][obj]["common series"][series["top"]], dates[meas], tmin, tmax, obj, agg_corr)
                bot[source][obj][meas]                   = subset_to_cutoff(data_dt[source][ty][meas][obj]["common series"][series["bot"]], dates[meas], tmin, tmax, obj, agg_corr)
                mid[source][obj][meas]                   = subset_to_cutoff(data_dt[source][ty][meas][obj]["common series"][series["mid"]], dates[meas], tmin, tmax, obj, agg_corr)
            ...
            for (dist_label, dist) in series_dict
                ...
                for (j, source) in enumerate(data_sources)
                    ...
                    if all(isnan.(dist[source][obj][meas]))
                        ...
                        s_data,
                        xlabel = L"\textrm{Year}",
                        ylabel = obj == "quantiles" ? L"\textrm{%$(M)\,\,(rel.\,  to\,\,  HH\,\,  average)}" : L"\textrm{%$(M)}\,\,\textrm{%$(obj)}",
                        xformatter=:latex, 
                        yformatter=:latex, 
