...
function run_TW_algorithm(MV, pr, block_type)
    ...
    Ftall, Λtall = perform_pca_missing(Matrix(tall_block'), 0.99) # Ftall is a r by T matrix

    # Step 4: Estimate loadings from the wide block using PCA
    Fwide, Λwide = perform_pca_missing(Matrix(wide_block'), 0.99)
    ...
    outdim = pr == 0.80 ? 10 : 12
    X_pc, X_proj = perform_pca_missing_outdim(X_imputed, outdim)
...
function perform_pca_missing_outdim(X_pca, outdim)

    M = MultivariateStats.fit(PCA, X_pca; maxoutdim=outdim, method=:svd) # mean=0
    pcs = MultivariateStats.transform(M, X_pca) # predict() produces the same # r by T
