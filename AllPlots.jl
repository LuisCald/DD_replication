...
function gen_proof_of_concept_copulas(d_data_dict::Dict, r_data_dict::Dict, source_of_id, measures, model_options, time_p, fully_observed_id, estimation_id)
    ...
    Plots.plot(
        ...
        xformatter=:latex, 
        yformatter=:latex, 
        xticks=(eachindex(b4_cov_ids), [L"%$(date)"[1:5] * "\$" for (_,date) in enumerate(dts[b4_cov_T_ids])]),
        linewidth=4,
        xtickfontsize=10,
        ytickfontsize=10,
        legendfontsize=10,
    ...
    Plots.plot(
        ...
        xformatter=:latex, 
        yformatter=:latex, 
        xticks=(eachindex(the2010s_ids), [L"%$(date)"[1:5] * "\$" for (_,date) in enumerate(dts[the2010s_T_ids])]),
        linewidth=4,
        xtickfontsize=10,
        ytickfontsize=10,
        legendfontsize=10,
...
function gen_proof_of_concept_figure(d_data_dict::Dict, r_data_dict::Dict, model_options, time_p, estimation_id, years)
    @unpack measures, equivalized, bottom_coded, blind_to, estimator = model_options
    @unpack tmin, tmax = time_p

    if typeof(estimator) <: SeriesEstimator @unpack integral_pcf_grid = estimator else @unpack grid_pcf, grid_cop = estimator end
                    ...
                    for (i, j) in enumerate(sequence)
                        ...
                        if M == "Consum"
                            ...
                            elseif j >= 7 && j <= 9
                                Plots.annotate!(sxaxis[end-10], sdata[indices...][end-4], Plots.text(label_q(j), fs, :bottom))
                            elseif j == 6
                                Plots.annotate!(sxaxis[end-10], sdata[indices...][end-10] .* .85, Plots.text(label_q(j), fs, :bottom))
                            else
                        ...
                        elseif M == "Wealth"
                            ...
                            elseif j >= 6 && j <= 9
                                if j == 6
                                    Plots.annotate!(sxaxis[end-7], sdata[indices...][end-7] .* .5, Plots.text(label_q(j), fs, :bottom))
                                else    
                                    Plots.annotate!(sxaxis[end-3], sdata[indices...][end] .* .8, Plots.text(label_q(j), fs, :bottom))
                                end
                            else
                        ...
                        if i != l_seq
                            Plots.scatter!(sxaxis, sdata[indices...], marker=:square, mc=:white, msc=:blue, msw=4, ms=6, label= "", dpi=500)
                        else
                            Plots.scatter!(sxaxis, sdata[indices...], marker=:square, mc=:white, msc=:blue, msw=4, ms=6, label= "", dpi=500)
                            Plots.scatter!([], [], marker=:square, mc=:white, msc=:blue, msw=4, label= L"\textrm{Data}", dpi=500)
                        end

                        # circles = circlemaker.(sxaxis, sdata[indices...], (0.12,)) 
                        # if i != l_seq 
                        #     Plots.plot!(circles, ratio=1, fc=:transparent, lc=:black, lw=2, label= "", dpi=500)
                        # else
                        #     Plots.plot!(circles, ratio=1, fc=:transparent, lc=:black, lw=2, label= "", dpi=500)
                        #     Plots.scatter!([], [], mc=:white, msc=:black, msw=3, label= L"\textrm{Data}", dpi=500)
                        #         # yerror=(sdata[indices...] - sci_l, sci_u - sdata[indices...])
                        # end
                    end
                    Plots.scatter!(
                        sxaxis, 
                        ...
                        # xformatter=:latex, 
                        # yformatter=:latex, 
                        # xticks=(sxaxis[1:3:end], [L"%$(date)"[1:5] * "\$" for (_,date) in enumerate(dts_to_keep[cond][1:3:end])]),
                        # xtickfontsize=10,
                        # ytickfontsize=10,
                        # legendfontsize=10,
                        # guidefontsize=14,
