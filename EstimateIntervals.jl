...
function define_data_intervals_only(df_vec, method_options, init_path, time_p, obs_data, intervals_for)
    ...
    if ci_exists && noise_exists
        ...
        for source in collect(keys(new_df_dict))
            ...
            confidence_intervals[source]["ci_u"], confidence_intervals[source]["ci_l"]    = construct_confidence_intervals(sub_boot_dict, .005, .995, objects, measures, length(year_vec[j]), source)
        end
...
run_interval_estimation(obs_data, method_options, ["CEX_all"])
