...
function draw_from_prior(param_sizes, priors, nchain)
    ...
    A_B = rand(priors[1], nchain)  # a matrix 
    Ω   = hcat([rand(priors[2], param_sizes[3][1]) for _ in 1:nchain]...)
    Σ   = hcat([rand(priors[3], param_sizes[4][1]) for _ in 1:nchain]...)
    θ   = vcat(A_B, Ω, Σ)
    return θ
end
...
function tune_scale(scale, init_scale, accepted, i)
    accpt_r = accepted / i
    if accpt_r > .2 && accpt_r <= .5
        return scale, init_scale 
    else
        scale      .= scale ./ init_scale
        if accpt_r <= .001
            init_scale *= 0.1
        elseif accpt_r > .001 && accpt_r <= .05
            init_scale *= 0.5
        elseif accpt_r > .05 && accpt_r <= .2
            init_scale *= 0.9
        elseif accpt_r > .5 && accpt_r <=.75
            init_scale *= 1.1
        elseif accpt_r > .75 && accpt_r <= .90
            init_scale *= 2
        elseif accpt_r > .90
            init_scale *= 10
        end
