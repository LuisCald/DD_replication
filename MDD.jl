...
function mdd_harmonic_mean(chain, lprobs; α=0.05, verbose=false, debug=false)
    # Flatten the chain if it has more than 2 dimensions
    if ndims(chain) > 2
        chain = reshape(chain, size(chain)[1] * size(chain)[2], size(chain)[end])
    end
    ...
    cmean = mean(chain, dims=1) |> vec
    ccov = cov(chain, dims=1)
    cicov = inv(ccov)  # Inverse of covariance matrix

    nsamples = size(chain, 1)
    d = size(chain, 2)  # Dimension of parameter vector
...
function bridge_sampler(model_dict, likeli, model_elements, param_sizes, priors, meas_ind, Σ_ids, model_options; cores=1, tol=1e-10, r0=0)
    bridge_dict = Dict()
    for k in keys(model_dict)
        bridge_dict[k] = Dict()
        samples = model_dict[k]["d_chains"]
        ...
        samples_4_iter = vcat([samples[nperchain-iter_index:nperchain, i, :] for i in 1:nchains]...) # making one long chain from last iteration
        lprobs_sub = model_dict[k]["lprobs"][nperchain-iter_index:nperchain, :][:]
        ...
        proposal_dist = model_dict[k]["propdist"] #MvNormal(m, V)
        ...
        bridge_dict[k]["logml"] = log(r) + lstar
    end
