...
function perform_extensive_forecast(dataset_choice, par_final, param_sizes, priors, meas_ind, Σ_ids, filtering_criteria, obs_data, model_options, model_elements, time_params, user_params, func_data, type, forecast_type)
    ...
    elseif forecast_type[2] == "data_only"
        ...
        n = Int(size(y, 1) / length(df_vec.df_names))
                ...
                for (j, source) in enumerate(data_sources)
                    ...
                    if c_data != []
                        ...
                        # if data_bounds != false
                            Plots.plot!(p_xaxis,
                            p_data,
                            ylabel = obj == "quantiles" ? L"\textrm{%$(M)\, \, rel.\,  to\,\,  HH\,\,   average}" : L"\textrm{%$(M)\,\,%$(series[ds]) \,\,%$(obj)}",
                            xformatter=:latex, 
                            yformatter=:latex, 
                            xticks=(p_xaxis[1:20:end], [L"%$(date)"[1:5] * "\$" for (_,date) in enumerate(p_xaxis[1:20:end])]),
                            legend=:best,
                            xtickfontsize=10,
                            ytickfontsize=10,
                            legendfontsize=10,
                        ...
                        Plots.scatter!(p_xaxis[id_for_missing], # findall(!isnan, top[source * "_obs"][obj][meas])
                            ...
                            lw=4, dpi=500,
                            label=""
                            # label= ds == "bot" ? L"\textrm{Missing}" : "",
                            # yerror = (c_data[within_data_missing_ids] - dist[1][source]["lb"][obj][meas][cond][within_data_missing_ids], dist[1][source]["ub"][obj][meas][cond][within_data_missing_ids] - c_data[within_data_missing_ids]),
                        )
