...
`smoothed_factors.csv` is the **point estimate** (posterior mode). To carry the
model's posterior uncertainty into your own moments, use
`smoothed_factor_draws.csv`: it stacks the smoothed factors over draws of the
model parameters, so evaluating a moment on each draw gives a posterior band.
The ready-made helpers `code/moments_from_coefficients/posterior_bands.jl`
(and `.py`) and `plot_factor_bands.jl` do exactly this. The draws file is
generated by `code/julia/export_draws.jl`; the raw parameter draws themselves
(`posterior_draws/*.jld2`) are large and not shipped in git.
